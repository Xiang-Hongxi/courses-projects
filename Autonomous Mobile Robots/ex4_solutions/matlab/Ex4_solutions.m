...
% SOLUTION.
alpha_x = (img_size_x/2.0)/tan(fov_x_rad/2.0); %555; %554.2562;
alpha_y = (img_size_y/2.0)/tan(fov_y_rad/2.0); %555; %55579.4113;
u0 = img_size_x/2.0;
v0 = img_size_y/2.0;
K = [alpha_x, 0, u0; 0, alpha_y, v0; 0, 0, 1];
R = [1, 0, 0; 0, 0, -1; 0, 1, 0];
t = [0; 1.5; 5.0];
...
function [u, v] = projectToImage(K, R, t, Pw, img_size_x, img_size_y)
    ...
    if u > img_size_x || u < 0 || v > img_size_y || v < 0
        u = nan;
        v = nan;
    end
end
