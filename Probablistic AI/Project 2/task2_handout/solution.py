    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                log_likelihood = -F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y.long(), reduction='sum')
                log_prior, log_variational_posterior, log_likelihood = log_prior / self.batch_size, log_variational_posterior / self.batch_size, log_likelihood / self.batch_size
                loss = 2. ** ( num_batches - batch_idx - 1. ) / ( 2. ** num_batches - 1. ) * (log_variational_posterior - log_prior) - log_likelihood
                loss.backward()
                self.optimizer.step()
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        # self.prior = None
        self.prior = UnivariateGaussian(mu=torch.tensor(0.0), sigma=torch.tensor(np.exp(-2)))
        # self.prior = UnivariateGaussian(mu=torch.tensor(0.0), sigma=torch.tensor(0.25*np.exp(-2)+0.25*np.exp(-6)))
        ...
        self.weights_var_posterior = MultivariateDiagonalGaussian(mu=torch.nn.Parameter(torch.zeros((out_features, in_features))), rho=torch.nn.Parameter(torch.ones((out_features, in_features)) * (-5))) 
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = MultivariateDiagonalGaussian(mu=torch.nn.Parameter(torch.zeros((out_features))), rho=torch.nn.Parameter(torch.ones((out_features)) * (-5)))
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            ...
            log_prior = log_prior + log_prior_layer
            log_variational_posterior = log_variational_posterior + log_variational_posterior_layer
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        return torch.log(1 / (self.sigma * np.sqrt(2 * np.pi)) * torch.exp(-0.5 * ((values - self.mu) / self.sigma) ** 2)).sum()
    ...
    def sample(self) -> torch.Tensor:
        ...
        # for the Univariate Gaussian distribution. 
        # raise NotImplementedError()
        standard_samples = torch.Tensor(self.mu.size()).normal_()
        samples = self.mu + standard_samples * self.sigma
        return samples
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        log_likelihood = torch.log(1 / (sigma * np.sqrt(2 * np.pi)) * torch.exp(-0.5 * ((values - self.mu) / sigma) ** 2)).sum()
    ...
    def sample(self) -> torch.Tensor:
        ...
        sigma = torch.log(1 + torch.exp(self.rho))
        standard_samples = torch.Tensor(self.mu.size()).normal_()
        samples = torch.zeros(self.mu.size()) #store samples
        samples = self.mu + standard_samples * sigma
        return samples
