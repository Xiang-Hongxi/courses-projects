...
class Agent:
    ...
    def get_action(self, obs):
        ...
        return self.step(torch.as_tensor(obs, dtype=torch.float32))[0]
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        _, logp = agent.actor(obs, act)
        loss_p = torch.mul(-adv, logp).mean()
        loss_p.backward()
        ...
        for _ in range(100):
            ...
            v = agent.critic(obs)
            loss_v = torch.sum(torch.square(v-ret))
            loss_v.backward()
            critic_optimizer.step()
