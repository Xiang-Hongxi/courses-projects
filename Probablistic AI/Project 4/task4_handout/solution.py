...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    multi_layer_perceptron = nn.Sequential()
    for idx,ele in enumerate(sizes):
        if idx==len(sizes)-1:
            break
        multi_layer_perceptron.add_module('layer_{}'.format(idx),nn.Linear(ele,sizes[idx+1]))
        if idx==len(sizes)-2:
            multi_layer_perceptron.add_module('act_{}'.format(idx),output_activation())
        else:
            multi_layer_perceptron.add_module('act_{}'.format(idx),activation())
...
class Actor(nn.Module):
    ...
    def _distribution(self, obs):
        ...
        pi = torch.distributions.Categorical(logits=log_prob)
        return pi
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        self.adv_buf[path_slice] = discount_cumsum(self.tdres_buf[path_slice],self.gamma*self.lam)
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                    tdres=self.tdres_buf, logp=self.logp_buf,adv=self.adv_buf)
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        _,log_prob = agent.actor.forward(data_obs,data_act)
        loss_pi = -torch.mul(data_adv,log_prob).mean()
        loss_pi.backward()
        ...
        for _ in range(100):
            ...
            v = agent.critic.forward(data_obs)
            loss_v = (v-data_ret).pow(2).sum()
            loss_v.backward()
            critic_optimizer.step()
