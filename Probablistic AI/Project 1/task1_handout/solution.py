...
class Model(object):
    ...
    def __init__(self):
        ...
        # self.kernel = RBF(length_scale=1e-3,length_scale_bounds="fixed")
        self.kernel = Matern(nu=2.5 , length_scale = 1, length_scale_bounds="fixed")
    ...
    def fitting_model(self, train_GT: np.ndarray,train_features: np.ndarray):
        ...
        training_indices = self.rng.choice(np.arange(train_GT.size),size=round(0.5*train_GT.size), replace=False)
        X_train, y_train = train_features[training_indices], train_GT[training_indices]
