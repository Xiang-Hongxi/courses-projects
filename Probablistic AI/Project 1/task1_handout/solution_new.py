...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel = [1 * RBF(length_scale=1.0, length_scale_bounds=(1e-5, 100000.0)),
                        RationalQuadratic(length_scale=1.0, alpha=0.1, alpha_bounds=(1e-5, 1e15)),
                        Matern(nu=1.5, length_scale=1, length_scale_bounds="fixed"),
                        Matern(nu=2.5, length_scale=1, length_scale_bounds="fixed")
        ]
        self.kernel_ = 1 * RBF(length_scale=1.0, length_scale_bounds=(1e-05, 100000.0))
        # (1e-08, 100000.0)
        # self.kernel2 = RationalQuadratic(length_scale=1.0, alpha=0.1, alpha_bounds=(1e-5, 1e15))
        # self.kernel3 = Matern(nu=1.5, length_scale=1, length_scale_bounds="fixed")
        # self.kernel4 = Matern(nu=2.5, length_scale=1, length_scale_bounds="fixed")
        # self.kernel4 = Matern(nu=2.5, length_scale=20, length_scale_bounds="fixed")
        self.model = GaussianProcessRegressor(kernel= self.kernel[0], random_state=self.rng.integers(10))
    ...
    def make_predictions(self, test_features: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        gp_mean, gp_std = self.model.predict(test_features, return_std=True, return_cov=False)
    ...
    def fitting_model(self, train_GT: np.ndarray,train_features: np.ndarray):
        ...
        # Undersample
        part_idx = self.rng.choice(np.arange(train_GT.size), size=round(0.2*train_GT.size), replace=False)
        X_part, y_part = train_features[part_idx], train_GT[part_idx]

        likelihood = 0
        for i in range(len(self.kernel)):
            self.model = GaussianProcessRegressor(kernel= self.kernel[i], random_state=self.rng.integers(10))
            self.model.fit(X_part, y_part)
            print(i,':', self.model.log_marginal_likelihood_value_)
            if likelihood > self.model.log_marginal_likelihood_value_:
                self.kernel_ = self.kernel[i]
                likelihood = self.model.log_marginal_likelihood_value_
        print(self.kernel_)
        train_idx = self.rng.choice(np.arange(train_GT.size), size=round(0.8*train_GT.size), replace=False)
        X_train, y_train = train_features[train_idx], train_GT[train_idx]
        
        self.model = GaussianProcessRegressor(kernel= self.kernel_, n_restarts_optimizer=3, random_state=self.rng.integers(10))
