...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    w = cellWidth
    h = cellHeight
    angleInterval = 2*np.pi/nBins # Interval size of angle in histogram
            ...
            for cell_x in range(-2, 2):
                ...
                for j in range(h):
                    for i in range(w):
                        ...
                        current_pixel_total_grad = np.sqrt(np.power(current_pixel_grad_x,2)+np.power(current_pixel_grad_y,2)) # weighted vote
